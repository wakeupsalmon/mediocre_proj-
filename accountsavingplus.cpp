...
void savingsaccount::addinterest(){
	account::deposit(interestrate * getbalance()); 
}
...
void chequingaccount::write(ostream &out){
	account::write(out);
	out<<"remaining free withdrawls:"<<remainingfreewithdrawals<<"\n";
}
...
bool chequingaccount::withdraw(float amount){
	...
	if(remainingfreewithdrawals > 0){
		value=amount-getwithdrawfee();
	}
	
	if(result && (remainingfreewithdrawals>0) ){
...
void savingsaccountplus::write(ostream &out){
	...
	out<<"minimum balance is: "<<minimumbalance<<"\n";
}
